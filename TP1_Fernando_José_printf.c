...
void* corrida(void* arg) {
    ...
    // Loop da corrida com impressão de progresso
    for (int i = 1; i <= LIMITE; i++) {
        printf("[%s] está no metro: %d\n", data->nome, i);
    ...
    if (!venceu) {
        ...
        venceu = 1;
        printf("\n*** %s CRUZOU A LINHA DE CHEGADA EM PRIMEIRO! ***\n\n", data->nome);
    }
...
int main() {
    ...
    printf("\n--- RESULTADO FINAL ---\n");
    printf("O vencedor oficial foi: %s!\n", nome_vencedor);
